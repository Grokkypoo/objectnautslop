...
9. **Moonwalk check before shipping.** In-place walk/run videos often generate a reverse treadmill gait (face right, legs cycle as if walking left). ffmpeg and the sampler do **not** reverse frames — if the GIF moonwalks, the video did. Re-sample with `--reverse` rather than guessing the extractor is backwards. Apply the same fix to every character that used that clip, not just the one the user noticed.
...
- `--reverse` if the preview GIF moonwalks (see QC). Do not default this on.
...
To flip a moonwalking set without re-extracting:
...
- [ ] **Gait matches facing (no moonwalk).** Character faces right → supporting legs should push *left* (backward along the ground) so a right-facing cycle would move the body right. If the GIF looks like walking the wrong way, pass `--reverse` and re-check. This is the most common in-place video failure; it is not an ffmpeg/index bug.
