...
| Gait pushes opposite the facing dir | Moonwalk / reverse treadmill |

Walk/run prompts must say the legs push **backward** (for a right-facing character, rear legs push left) so the cycle would carry them to the right if they were not on a treadmill.

Run example:

```text
The same chibi ninja character runs in place facing right with a classic side-scroller
stride, rear foot pushing left, arms trailing slightly back, body centered, camera
locked, solid flat magenta background only. Not a moonwalk.
```

Walk / idle examples:

```text
The character walks in place facing right with a steady side-view walk cycle, each
step pushing backward so the gait would move them right, camera locked, solid magenta
background. Not a moonwalk.
```
...
- **Moonwalk:** even sampling is chronological (index 0 → last). If the GIF moonwalks, reverse the sample with `--reverse`. Do not assume the extractor stored frames backwards. The video model generated a reverse gait because in-place motion has no translation to constrain direction.
